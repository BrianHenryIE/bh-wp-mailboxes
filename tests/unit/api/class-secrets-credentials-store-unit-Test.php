 ...
 *
 * The round trips against the real feature plugin are in the wpunit test; these cover the
 * serialisation, naming, and the error paths (unavailable API, WP_Error, malformed records).
 *
...
class Secrets_Credentials_Store_Unit_Test extends Unit_Testcase {
	...
	/**
	 * A store whose is_available() is the real function_exists() check.
	 */
	 ...
	 * wp_delete_secret() answers $delete_result.
	 *
	 ...
	 */
	protected function mock_secrets_api( mixed $stored = null, mixed $set_result = true, mixed $delete_result = true ): void {
		$this->stored        = $stored;
		...
		WP_Mock::userFunction( 'wp_delete_secret' )->andReturnUsing( fn() => $this->delete_result );
	}
	...
	/**
	 * The availability check is function_exists() on the three Secrets API functions. (Only the positive case can be
	 * asserted here: once any test in the process has mocked them they stay defined.)
	 *
	 * @covers ::is_available
	 */
	public function test_is_available_with_the_secrets_api(): void {
	...
	public function test_get_logs_and_returns_null_on_wp_error(): void {
		$this->mock_secrets_api( new WP_Error( 'secret_key_unavailable', 'Keyring broken.' ) );
	...
	public function test_get_logs_and_returns_null_for_invalid_json(): void {
		$this->mock_secrets_api( 'not json' );
	...
	public function test_get_logs_and_returns_null_for_an_unknown_type(): void {
		$this->mock_secrets_api( '{"type":"pop3"}' );
	...
	public function test_get_logs_and_returns_null_for_gmail_without_a_client(): void {
		$this->mock_secrets_api( '{"type":"gmail","access_token":null}' );
	...
	public function test_imap_round_trip(): void {
		...
		$this->mock_secrets_api( $this->written[ $name ] );
		...
		$this->mock_secrets_api( '{"type":"imap","server":"s","username":"u","password":"p"}' );
	...
	public function test_save_throws_and_logs_on_wp_error(): void {
		$this->mock_secrets_api( null, new WP_Error( 'secret_key_unavailable', 'WP_SECRETS_KEY is not defined.' ) );
	...
	public function test_delete_throws_and_logs_on_wp_error(): void {
		$this->mock_secrets_api( null, true, new WP_Error( 'secret_store_unavailable', 'Store down.' ) );
